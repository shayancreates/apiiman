            ...
            if (currentTop >= openThreshold) {{
                ...
                animateTo(maxY, () => {{
                    ...
                    if (chatbotIframe.src === "about:blank") {{
                        // Send message to parent (Streamlit) that zipper is fully open
                        window.parent.postMessage({{type: 'zipperOpened'}}, '*');
                    }}
                }});
            }} else {{
        ...
        function init() {{
            ...
            window.addEventListener('message', function(event) {{
                if (event.data.type === 'zipperOpened') {{
                    // This is just for demonstration - in reality the parent would send this
                    console.log('Received zipper opened message');
                }}
            }});
        }}
    ...
    if not st.session_state.zipper_opened:
        ...
        components.html(
            ...
            <script>
                window.addEventListener('message', function(event) {
                    if (event.data.type === 'zipperOpened') {
                        // Send message to Streamlit to update the session state
